<template>
  <div>
    ...
    <div class="flex h-screen">
      <!-- Sidebar -->
      <div class="flex rounded border-r border-gray-lightest">
        ...
        <div :class="isOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed z-30 inset-y-0 left-0 w-64 transition duration-300 transform bg-primary-lightest overflow-y-auto lg:translate-x-0 lg:static lg:inset-0">
          ...
          <nav class="mt-10 font-semibold text-lg text-gray-dark">
            <router-link
              class="flex items-center duration-200 p-3 px-6 m-3"
              :class="[$route.name === 'Staging' ? activeClass : inactiveClass]"
              to="/dashboard"
            >
      ...
      <main class="flex-1 overflow-x-hidden overflow-y-auto bg-primary-lightest">
...
export default {
    data: () => ({
        title: 'Dashboard',
        activeClass: 'text-white bg-primary-base hover:bg-gray-700 rounded-lg'
    }),
    components: {
      // PageHeader
    }

<template>
  <div class="bg-white-base p-4 md:flex shadow-md relative">
    <div class="w-full">
    ...
    <div v-click-outside="closeAccountCardMenu" class="ml-auto absolute right-4">
      <a @click.prevent="toggleAccountCardMenu" class="text-gray-700 cursor-pointer">
        <svg :class="[{ transform: accountCardMenuOpen },{ '-rotate-45': accountCardMenuOpen}]" class="h-5 fill-current transition duration-500 ease-in-out" viewBox="0 0 16 16">
      ...
      <div v-show="accountCardMenuOpen" class="absolute right-0 mt-0 w-48 rounded-md shadow-lg py-1 bg-white-base" role="menu" aria-orientation="vertical">
  ...
  components: {
    ...
    methods: {
      toggleAccountCardMenu: function() {
        this.accountCardMenuOpen = !this.accountCardMenuOpen
        console.log(this.accountCardMenuOpen)
      },
      closeAccountCardMenu: function() {
        this.accountCardMenuOpen = false
      },
